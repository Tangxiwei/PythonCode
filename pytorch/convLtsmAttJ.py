...
lookback = 8
# 读取多个数据文件
file_paths = [r"D:\datasets\data03235.xlsx"]
# 数据预处理和创建数据集
def preprocess_data(file_path, truncation=0, lookback=8):
...
def train_and_predict(X_train, X_test, y_train, y_test, scaler, y_test_original, file_index, n_epochs=45):
    ...
    model = ConvLSTMWithAttention(input_size=input_size, hidden_size=256, output_size=output_size, num_layers=3,
    ...
    plt.text(0.1, 0.9, f'Test RMSE: {test_rmse_value:.4f}', horizontalalignment='center', verticalalignment='center',
...
for i, file_path in enumerate(file_paths):
    ...
    # 训练和预测
    train_and_predict(X_train, X_test, y_train, y_test, scaler, y_test_original, i + 323)
